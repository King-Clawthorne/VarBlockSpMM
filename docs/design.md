...
The row-owned family specializes only RHS width `{8,16,32,64}`. Runtime block dimensions remain loop bounds. RHS 8 assigns one output element per thread. RHS 16/32/64 assign four RHS columns of one local row to each thread, so one A load feeds four independent accumulators. A two-dimensional grid assigns CTAs disjoint output elements, so the path needs neither atomics nor workspace.
