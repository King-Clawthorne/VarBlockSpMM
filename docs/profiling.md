...
ncu --target-processes all --kernel-name-base demangled --kernel-name "regex:.*row_owned_single_buffered_direct.*" --launch-skip 3 --launch-count 1 --section SpeedOfLight --section MemoryWorkloadAnalysis --section Occupancy --section SchedulerStats --section WarpStateStats --csv --log-file data/ncu_high_staged.csv build\Release\vbsr_benchmark.exe --rows 4096 --degree 16 --rhs 64 --distribution bimodal --locality random --warmup 3 --reps 1
ncu --target-processes all --kernel-name-base demangled --kernel-name "regex:.*row_owned_double_buffered.*" --launch-skip 3 --launch-count 1 --section SpeedOfLight --section MemoryWorkloadAnalysis --section Occupancy --section SchedulerStats --section WarpStateStats --csv --log-file data/ncu_rhs32_double_buffered.csv build\Release\vbsr_benchmark.exe --rows 4096 --degree 16 --rhs 32 --distribution bimodal --locality random --warmup 3 --reps 1
```

The `ncu_high_ilp4.csv` command describes the historical four-accumulator revision. The final two commands target the retained RHS-64 single-buffer kernel and the RHS-32 double-buffer kernel respectively.
